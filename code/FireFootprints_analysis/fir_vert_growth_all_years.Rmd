...
LMdf <- df %>% 
  ...
  select(Fire, Sdlg, heatload, Years, BasDia2016.cm, shrubarea3, sqrt_shrubarea3, log_shrubarea3, Elevation, Ht_cm1, Slope.Deg, Year, Aspect.rad.fold.NESW, incidrad) %>% 
...
without <- lme(VertGrowth_Rel ~ 
           heatload +  Years + Aspect.rad.fold.NESW + incidrad + BasDia2016.cm + shrubarea3 + Year,
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
...
```{r}
LM <-  lme(VertGrowth_Rel ~ heatload +  Years + Aspect.rad.fold.NESW + incidrad + BasDia2016.cm + shrubarea3 + Year,
           data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
summary(LM)
drop1(LM, test = "Chisq")
```
