...
load(file="fire_footprints/master_seedlings_vert.Rdata")
```
...
## Check out shrubarea3 and cov1.2

### See how correlated they are
```{r}
ggplot(df, aes(x = shrubarea3, y = Cov1.2))+
  geom_point()+
...
LMdf <- df %>% 
  ...
  select(Sdlg, VertGrowth_Rel, shrubarea3, sqrt_shrubarea3, BasDia2016.cm, Year, Cov1.2, incidrad, Year) %>% 
...
```{r}
AIC(lme(VertGrowth_Rel ~ shrubarea3 + BasDia2016.cm + Cov1.2 + incidrad + Year,data = LMdf, random = ~ 1| Sdlg, method = "ML"))
AIC(lme(VertGrowth_Rel ~  sqrt_shrubarea3 + BasDia2016.cm + Cov1.2 + incidrad + Year,data = LMdf, random = ~ 1| Sdlg, method = "ML")
)
```
...
## Linear model
```{r}
LM <-  lme(VertGrowth_Rel ~ shrubarea3 + BasDia2016.cm + Cov1.2 + incidrad + Year,data = LMdf, random = ~ 1| Sdlg, method = "ML")
summary(LM)
drop1(LM, test = "Chisq")
```
