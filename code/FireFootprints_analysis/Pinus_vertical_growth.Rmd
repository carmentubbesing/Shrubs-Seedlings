...
LMdf <- df %>% 
  ...
  select(Fire, Sdlg, VertGrowth_Rel, Years, Ht_cm1, shrubarea3, sqrt_shrubarea3, log_shrubarea3, BasDia2016.cm, incidrad, Year, Elevation, heatload, ShrubSpp03) %>% 
...
```{r}
vars
LM <- lme(VertGrowth_Rel ~  Years +  Ht_cm1 + shrubarea3 + heatload + incidrad + Year,
...
AIC(lme(VertGrowth_Rel ~ 
         Years +  Ht_cm1 + shrubarea3 + heatload +  incidrad + Year, 
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML"))

AIC(lme(VertGrowth_Rel ~  
          Years +  Ht_cm1 + sqrt_shrubarea3 + heatload +  incidrad + Year, 
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML"))

AIC(lme(VertGrowth_Rel ~  
           Years +  Ht_cm1 + log_shrubarea3 + heatload +  incidrad + Year, 
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML"))
...
fit_max <- effects_df[which.min(abs(effects_df$shrubarea3 - max)),]$fit
