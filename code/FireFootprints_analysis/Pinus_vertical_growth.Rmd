...
## See how soil order tracks with shrub species & Fire
```{r}
df_count <- df %>% 
  group_by(soil_order, ShrubSpp03) %>% 
  summarize(n = n())
ggplot(df_count)+
  geom_col(aes(x = soil_order, y = n, fill = ShrubSpp03))
ggsave(file = "../../results/figures/FireFootprints/PineOrderShrubSpp.png", width = 6, height = 6, dpi = 400)
```

```{r}
df_count <- df %>% 
  group_by(soil_order, Fire) %>% 
  summarize(n = n())
ggplot(df_count)+
  geom_col(aes(x = Fire, y = n, fill = soil_order))
ggsave(file = "../../results/figures/FireFootprints/PineOrderFire.png", width = 6, height = 6, dpi = 400)
...
LMdf <- df %>% 
  ...
  select(Fire, Sdlg, VertGrowth_Rel, Years, Ht_cm1, shrubarea3, sqrt_shrubarea3, log_shrubarea3, BasDia2016.cm, incidrad, Year, Elevation, heatload, ShrubSpp03, soil_order) %>% 
...
```{r}
vars
LM <- lme(VertGrowth_Rel ~  Years +  Ht_cm1 + shrubarea3 + heatload + soil_order + incidrad + Year,
...
AIC(lme(VertGrowth_Rel ~ 
         Years +  Ht_cm1 + shrubarea3 + heatload + soil_order + incidrad + Year, 
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML"))

AIC(lme(VertGrowth_Rel ~  
          Years +  Ht_cm1 + sqrt_shrubarea3 + heatload + soil_order + incidrad + Year, 
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML"))

AIC(lme(VertGrowth_Rel ~  
           Years +  Ht_cm1 + log_shrubarea3 + heatload + soil_order + incidrad + Year, 
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML"))
...
eff_shrspp <- predictorEffect("soil_order", LM)
plot(eff_shrspp)
...
```{r}
summary <- summary(df$shrubarea3/10000)
quart1 <- summary[2]
quart3 <- summary[5]
median <- summary[3]
min <- summary[1]
...
fit_quart1 <- effects_df[which.min(abs(effects_df$shrubarea3 - quart1)),]$fit
fit_quart3 <- effects_df[which.min(abs(effects_df$shrubarea3 - quart3)),]$fit
fit_median <- effects_df[which.min(abs(effects_df$shrubarea3 - median)),]$fit
fit_min <- effects_df[which.min(abs(effects_df$shrubarea3 - min)),]$fit
fit_mean <- effects_df[which.min(abs(effects_df$shrubarea3 - mean)),]$fit
mean <- mean(df$shrubarea3/10000)
