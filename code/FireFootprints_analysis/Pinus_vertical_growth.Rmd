...
```{r}
vars
LM <- lme(VertGrowth_Rel ~  Years +  Ht_cm1 + sqrt_shrubarea3 + heatload + BasDia2016.cm + Year + ShrubSpp03,
...
```{r}
LM <- lme(VertGrowth_Rel ~  Ht_cm1 + sqrt_shrubarea3 + heatload + Years + ShrubSpp03 + siteclass + Year,
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
drop1(LM, test = "Chisq")
AIC(LM)
LM_int <- lme(VertGrowth_Rel ~  Ht_cm1*sqrt_shrubarea3 + heatload + Years + ShrubSpp03 + siteclass + Year,
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
AIC(LM_int)
```
...
without <- lme(VertGrowth_Rel ~ 
            Years +  Ht_cm1 + sqrt_shrubarea3 + heatload + BasDia2016.cm + Year + ShrubSpp03,
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
AIC(without)
with_ht <- lme(VertGrowth_Rel ~  
            Years +  Ht_cm1*sqrt_shrubarea3 + heatload + BasDia2016.cm + Year + ShrubSpp03,
        data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")

AIC(with_ht)

anova(with_ht, without)
```
...
# Re-establish the best LM so far and look at summary and drop1 results
```{r}
LM <-  lme(VertGrowth_Rel ~ 
            Years +  Ht_cm1*sqrt_shrubarea3 + heatload + BasDia2016.cm + Year + ShrubSpp03,
           data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
...
```{r}
LM_noshrub <- lme(VertGrowth_Rel ~ 
            Years +  Ht_cm1+ heatload + BasDia2016.cm + Year + ShrubSpp03,
           data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
anova(LM, LM_noshrub)
```

```{r}
LM_noheight <- lme(VertGrowth_Rel ~ 
            Years + sqrt_shrubarea3 + heatload + BasDia2016.cm + Year + ShrubSpp03,
           data = LMdf, random = ~ 1| Fire/Sdlg, method = "ML")
anova(LM, LM_noheight)
```
...
eff_shrubarea <- predictorEffect("sqrt_shrubarea3", LM, xlevels = list(Ht_cm1 = quartiles))
effects_df <- as.data.frame(eff_shrubarea)
head(effects_df)
...
## Plot effects of shrubarea WITHOUT interaction
