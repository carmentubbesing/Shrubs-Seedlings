...
# Take a look
```{r}
hist(as.numeric(df$DiaGrowth.mm))
summary(df$DiaGrowth.mm)
df %>% filter(is.na(DiaGrowth.mm))
```
...
```{r}
f <- DiaGrowth.mm_rel ~  shrubarea2_sqrt_norm + Ht_log_norm 
...
figure_summary <- ggplot(MyData_summary_simple)+
  ...
  geom_point(data=df_dia, aes(x = sqrt(shrubarea2), y = DiaGrowth.mm_rel))
dev.off()
figure_summary
