...
df %>% 
  arrange(Date) %>% 
  select(Sdlg, Date, Fire, DataSheet1, DataSheet2) %>% 
  View()
```
...
dfl <- df %>% 
  ...
  dplyr::select(Sdlg, Ht.cm, Fire, FirePatch, Elevation, Slope.Deg, Light_File,ShrubSpp03, Cov1.3, Ht1.3, Years, LAI.53, DIFN.53, ref)
```
...
# Take out 2 DIFN measurements = 0 because they don't make sense
```{r}
dfl <- dfl %>% 
  filter(DIFN.53 != 0)
nrow(dfl)
...
LM <- lm(DIFN.53 ~ Years +  shrubarea3 + ShrubSpp03, data = dfl)
summary(LM)
drop1(LM, test = "Chisq")
```
