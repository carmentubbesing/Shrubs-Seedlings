...
for(i in 1:length(files)){
  ...
  if("X" %in% names(table)){
    table <- table %>% select(X, Y, Name)  
  } else{
    table <- table %>% select(Title, Latitude, Longitude) %>% 
...
### Check
```{r}
df %>% filter(Sdlg == 227) %>% select(Sdlg, X, Y, file_lat)
table_n[1:4,]
```
...
df %>% 
  filter(is.na(X)) %>% 
  select(Sdlg, FirePatch, X, Species) %>% 
  distinct()
```
...
df %>% filter(Sdlg %in% c(105:109)) %>% select(Sdlg, X, Y) %>% distinct() %>% arrange(Sdlg) %>% print.data.frame 
```
...
df %>% 
  filter(is.na(Y)) %>% 
  select(Sdlg, FirePatch, X, Y, Species) %>% 
  distinct()
```
...
df %>% 
  ...
  select(Aspect.deg, Aspect.rad, Aspect.rad.fold.NS, Aspect.rad.fold.NESW, Slope.Deg, Slope.rad, Y, Lat.rad, heatload, incidrad)
```
