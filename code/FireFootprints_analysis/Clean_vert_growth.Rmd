...
### For those seedlings, find 2015 growth by assuming that Growth_16 was the full apical growth as of 2016 measurements and using the ratio of 2016 growth to 2015 growth as measured in 2016

### Find average 2016 growing season apical growth relative to 2015 growth
```{r}
find_apical_growth <- df %>%
  filter(!is.na(Ht2015.meas2016)) %>%
  mutate(apical_growth_2016 =
...
#### height
```{r}
ggplot(find_apical_growth %>% filter(apical_ratio<5), aes(x = Ht2015.meas2016, y = apical_ratio, col = Species)) +
  geom_point()+
  geom_smooth(method = "lm")
```

No clear trend, at least for PIPO.

#### date
```{r}
ggplot(find_apical_growth, aes(x = Date1_2016, y = apical_ratio, col = Species)) +
  geom_point()+
  geom_smooth(method = "lm")
```
...
```{r}
cov(as.integer(find_apical_growth$Date1_2016), find_apical_growth$apical_ratio)
...
df %>%
  ...
  dplyr::select(Species, Sdlg, Ht16_meas2016.cm, LastYearGrth.cm, Ht2015.meas2016, Ht2015.calc2016)
```
...
df %>% 
  filter(is.na(Ht2015.cm_spring)) %>% 
  dplyr::select(Sdlg, LastYearGrth.cm, Ht16_meas2016.cm, Ht2017.cm.fall, Ht2015.meas2016, Growth_17, Growth_16, Growth_15, notes)
```
...
df %>% 
  ...
  dplyr::select(Sdlg, LastYearGrth.cm, Ht16_meas2016.cm, Ht2017.cm.fall, Ht2015.meas2016, Growth_17, Growth_16, Growth_15, notes, Ht2016.cm_spring, Ht2017.cm_spring, Ht2015.cm_spring)
```
...
df %>% 
  filter(is.na(Ht_cm)) %>% 
  dplyr::select(Sdlg, LastYearGrth.cm, Ht2016.cm_spring, Ht2017.cm.fall, Ht2015.meas2016, Year, VertGrowth_cm, notes)
```
