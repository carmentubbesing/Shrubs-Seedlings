...
## Test cover and height - all should be TRUE
```{r}
sum(shr[shr$Sdlg==10 & shr$Seg==1,]$Cover.cm) == cov[cov$Sdlg==10,2]
sum(shr[shr$Sdlg==1 & shr$Seg==3,]$Cover.cm) == cov[cov$Sdlg==1,4]
mean(shr[shr$Sdlg==10 & shr$Seg==1,]$Ht.cm) == ht[ht$Sdlg==10,2]
mean(shr[shr$Sdlg==1 & shr$Seg==3,]$Ht.cm) == ht[ht$Sdlg==1,4]
```
...
```{r}
shr_by_sdlg <- shr_by_sdlg %>% 
  dplyr::select(-Cov1, -Cov2, -Cov3, -Ht1, -Ht2, -Ht3)
```
