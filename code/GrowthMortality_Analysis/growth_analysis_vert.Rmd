...
df %>% 
  ...
  ggplot+geom_histogram(aes(log_gr_rel_ave), bins = 20)
df %>% 
  filter(DEAD_ALIVE == "ALIVE") %>% 

ggplot()+
  geom_histogram(data = df %>% filter(DEAD_ALIVE=="DEAD"), aes(log_gr_rel_ave), bins = 20, alpha = .2, col = "red", fill = "red")+
  theme_bw()+
  geom_histogram(data = df %>% filter(DEAD_ALIVE=="ALIVE"), aes(log_gr_rel_ave), bins = 20, alpha = .2, col = "dark green", fill = "dark green")
  
ggsave("../../results/figures/GrowthMortality/histogram_dead_alive.png")
