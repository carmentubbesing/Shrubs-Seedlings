...
```{r}
df <- df %>% 
  dplyr::select(series, board, everything())
df_bu <- df
```
...
```{r}
if(Sys.info()[1]=="Darwin"){
  meta <- read_excel("../../Dropbox (Stephens Lab)/SORTIE/Growth_mortality/data/details/compiled/Mort_details_compiled_Dec13_2018.xlsx", sheet = "data")
} else{
  meta <- read_excel("../../../../Dropbox (Stephens Lab)/SORTIE/Growth_mortality/data/details/compiled/Mort_details_compiled_Dec13_2018.xlsx", sheet = "data")
}
```
...
for(i in 1:length(list)){
  ...
  if(length(grep("BU", series))>0){
    seriesnew <- series %>% mutate(series = gsub("BU", "",series))
    series <- bind_rows(series, seriesnew)
  }
  series
...
```{r}
names(rwl[,!(names(rwl) %in% join$tree)])
```
...
```{r}
save(join, file = "../../data/GrowthMortality/dendro_all_vars.Rdata")
```
...
### Check
```{r}
rwl_live_both_gr6_nochron[1:2]
rwl_live_abco_gr6_nochron[1:2]

ncol(rwl_live_both_gr6_nochron)
rwl_live_both_gr6_nochron[110:111]
ncol(rwl_live_pipo_gr6_nochron)
rwl_live_pipo_gr6_nochron[60:61]
```
