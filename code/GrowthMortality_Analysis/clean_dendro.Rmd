...
```{r}
meta <- meta %>% 
  mutate(thrown_out = ifelse(SEEDLING %in% c("58B", "53B", "7B", "28B", "22B", "189B", "188B", "185BA", "17B", "162B", "155B", "147B", "149B", "145B", "136B", "114B", "104B"), "?", thrown_out))
```

### Add column to df for mateless seedlings
```{r}
df <- df %>% 
  mutate(mateless = ifelse(tree %in% c("7B","76B", "58B", "53B", "5B", "28B", "22B", "189B", "188B", "185BA", "17B", "162B", "155B", "147B", "149B", "145B", "136B", "114B", "104B"), 1, 0)) %>% 
...
join %>% 
  ...
  dplyr::select(tree, PAIR, thrown_out, board) %>% 
  distinct() %>% 
  arrange(tree) %>% 
  filter(!is.na(board))
