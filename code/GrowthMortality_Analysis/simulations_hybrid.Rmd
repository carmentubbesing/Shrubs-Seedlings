...
ggplot()+
  ...
  geom_vline(aes(xintercept = exp(-1.41822)), col = "red")+
  geom_text(aes(x= exp(-1.41822), label="\nminimum fire footprint shrub competition", y = ymax*.6), colour="red", angle=90) +
  geom_vline(aes(xintercept = exp(-2.5906)), col = "red")+
  geom_text(aes(x= exp(-2.5906), label="\nmaximum fire footprint shrub competition", y = ymax*.6), colour="red", angle=90) +
  geom_vline(aes(xintercept = exp(-2.0161789)), col = "red")+
...
for(i in 1:1000){
  ...
  dfa_new <- dfa %>% 
    ...
    mutate(death_year_temp = ifelse(death_year == 2016, 
                                    sample(c(2016, 2017), 1, replace = T), death_year_temp)) %>% 
    mutate(death_year_temp = ifelse(death_year == 2015, 2015, death_year_temp)) %>% 
    ungroup()
...
ggplot()+
  ...
  ylim(0, ymax)+
  ylab("Annual % mortality probability")+
  #ggtitle("HYBRID SIMULATIONS Fir mortality probability in relation to growth\nbased on simulation results, with 95% confidence intervals")+
  xlab(bquote('Annual relative growth defined as '  ~ frac((height[2] - height[1]),height[1])))
ggsave("../../results/figures/GrowthMortality/predict_mortality_abco_hybrid.png", height = 5, width = 6)
```
