...
ringplot(102)
look(102) 
add2016(102) # based largely on photo and ppt
...
```{r}
add2016(4) # based on ppt

ringplot("184-A")
add2016("184-A") # based on ppt
...
ringplot(18)
add2016(18) # based on ppt

ringplot("183-A")
look("183-A")
add2016("183-A") # based on ppt

ringplot(197)
add2015(197) # based on physical characteristics

ringplot(198)
add2017(198) # based largely on photos + ppt

ringplot(207)
add2016(207) # based on ppt
...
```{r}
add2017(232) # looks like it actually is growing
```
...
## Find duplicates
```{r}
length(unique(c(died2017, died2016, died2015))) == length(unique(dfa$tree))
```
...
bark2017 <- dfafull %>% 
  ...
  group_by(BARK) %>% 
  summarize(n = n())
bark2017[bark2017$BARK=="intact","n"]/sum(bark2017$n)
...
BUDS2017 <- dfafull %>% 
  filter(tree %in% died2017) %>%
  dplyr::select(tree, BUDS) %>% 
  distinct() %>% 
  mutate(BUDS = tolower(BUDS)) %>% 
  group_by(BUDS) %>% 
  summarize(n = n())
BUDS2017
BUDS2017[BUDS2017$BUDS=="intact","n"]/sum(BUDS2017$n)
...
ggsave("../../results/figures/GrowthMortality/dead_chron_abco.jpg", width = 3.5, height = 3.5)
```
...
# List trees in each year of death
```{r}
sort(died2017)
sort(died2016)
sort(died2015)
sort(definitely_green)
```
