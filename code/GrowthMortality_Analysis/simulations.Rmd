...
for(i in 1:1000){
  ...
  r2.theor <- c(r2.theor, r.squaredGLMM(glm)[1,1])
  r2.delta <- c(r2.delta, r.squaredGLMM(glm)[2,1])
}
mean_coef_gr <- mean(coefficient_gr)
mean_coef_int <- mean(coefficient_int)
mean_r2.theor <- mean(r2.theor)
mean_r2.delta <- mean(r2.delta)
mean_r2.theor
mean_r2.delta
```

## Plot one iteration of the simulation
```{r}
df$pred <- predict(glm, type = "response")
ggplot(df)+
  geom_boxplot(aes(x = as.factor(dead), y = pred))
```
