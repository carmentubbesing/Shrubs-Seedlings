...
```{r}
lm <- lm(relgrdia ~ dia17_cm_Aug + height_2018 + species + shrubarea, data = df)
summary(lm)
```
...
## ABCO
```{r}
lmabco <- lm(dia18_cm ~ sqrt(shrubarea) + height_2018 * dia17_cm_Aug, data = df %>% filter(species == "ABCO"))
summary(lmabco)
drop1(lmabco, test = "F")
```
