...
## How many trees are neither emerged nor dead at the end?
```{r}
dfsimallreps %>% 
...
### Counts
```{r}
emerge_count <- ggplot(summary_emerge)+
  theme_minimal()+
...
```{r}
emerge_prop <- ggplot(summary_emerge_prop)+
  theme_minimal()+
...
```{r}
grid.arrange(emerge_count, emerge_prop, ncol = 1)
```
...
dfsimallreps %>% 
  ...
  dplyr::select(Sdlg, Ht1.3, Ht_cm1) %>% 
...
boxplot_trees <- ggplot(summary_count)+
  ...
  theme_minimal()+
  ylab("Number of emerged trees")+
  ggtitle(paste("Results for", iterations, "simulations at", years+9,"years after fire", sep = " "))
```
