...
test <- dfsimall %>% 
  ...
  select(Sdlg, ID_withinrep, Ht1.3, Ht_cm1, Years, emerged) 
...
summary_emerge <- dfsimallreps %>% 
  ...
  arrange(Years) %>% 
  mutate(total_emergents = sum(new_emergents))
```
...
ggplot(summary_prop %>% filter(stat %in% c("prop_total", "prop_emerged")))+
  geom_boxplot(aes(x = Species, y = value, col = stat))+
  theme_minimal()+
  ylab("Number of trees")+
  ggtitle("Simulation results at 29 years post-fire")
