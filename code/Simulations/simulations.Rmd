...
```{r}
test <- dfsimallreps %>% 
  filter(Years %in% c(9,10)) %>% 
  dplyr::select(Sdlg, Species, Ht_cm1, dia.cm, Years, pred_exp, rep) %>% 
  tbl_df() %>% 
  arrange(Sdlg)
test
test <- test %>% 
  mutate(ID = paste(Sdlg, rep, sep = "-"))
test
...
test %>% 
  group_by(Years) %>% 
  count()
plot(test[test$Years==9, "dia.cm"], test[test$Years==10, "dia.cm"])
```
