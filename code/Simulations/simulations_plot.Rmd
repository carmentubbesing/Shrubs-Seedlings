...
```{r}
load("~/Simulations_2020-02-12_1000_.Rdata")
strt <- Sys.time()
```
...
## Shrub height growth patterns
```{r}
dfsimallreps_summary <- dfsimallreps %>% 
  ungroup() %>%
  mutate(rep = as.factor(paste(rep))) %>% 
  group_by(rep, Years, ShrubSpp03) %>% 
  mutate(mean_shrub_ht = mean(Ht1.3))
```

```{r}
ggplot(dfsimallreps_summary)+
  geom_smooth(aes(x = Years, y = mean_shrub_ht, col = ShrubSpp03, group = interaction(rep, ShrubSpp03)))+
  theme_bw()
```
...
ggplot(dfsimallreps_summary, aes(x = Years, y = mean_shrub_ht, group = ShrubSpp03, col = ShrubSpp03))+
  ...
  geom_smooth()+
  ggtitle(paste("Results for", max(dfsimallreps$Years), "simulations", sep = " "))+
  xlab("Years since fire")+
  ylab("Shrub height")+
  theme_bw()
```
...
## Plot number proportion of original seedlings left at the end
