...
```{r}
load("~/Simulations2020-02-28_50AMRC.Rdata")
strt <- Sys.time()
```
...
diabox <- ggplot(dfsimallreps_summary)+
  geom_boxplot(aes(x = as.factor(Years), y = dia.cm, fill = Species), outlier.shape = NA)+
  theme_minimal()+
  xlab("Years")+
  ylab("Diameter (cm)")+
...
htbox <- ggplot(dfsimallreps)+
  geom_boxplot(aes(x = as.factor(Years), y = Ht_cm1, fill = Species), outlier.shape = NA)+
  theme_minimal()+
  xlab("Years")+
  ylab("Height (cm)")+
  theme(text = element_text(size = 20))+
