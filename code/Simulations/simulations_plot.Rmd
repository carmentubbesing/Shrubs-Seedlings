...
```{r}
name <- "2020-03-04_1000AMRC_random"
...
#### Figure out what's up with abrupt growth of STAR ABCO
```{r}
summary_emerge %>% filter(Years %in% c(25,26)) %>% filter(Species == "ABCO") %>% arrange(desc(prop_emerged)) %>% 
  ggplot()+
  geom_histogram(aes(x = prop_emerged, fill = as.factor(Years)), alpha = 0.5)+
  facet_grid(. ~ Years)+
  theme_minimal()
```

```{r}
summary_emerge %>% filter(Years %in% c(25,26)) %>% filter(Species == "ABCO") %>% 
  group_by(Years) %>% 
  count()
```

```{r}
summary_emerge %>% filter(Years %in% c(25,26)) %>% filter(Species == "ABCO") %>% group_by(rep) %>% mutate(diff = diff(prop_emerged)) %>% arrange(desc(diff))
```
...
dfsimallreps %>% 
  filter(rep == 114) %>% 
  ggplot()+
  geom_line(aes(x = Years, y = Ht_cm1, group = Sdlg), col = "red")+
  theme_minimal()+
  geom_line(aes(x = Years, y = Ht1.3, group = Sdlg), col = "blue")
```


```{r}
plot_emerge_prop <- ggplot(summary_emerge %>% filter(Years <44))+
  theme_minimal()+
  ...
  xlab(element_blank())+
  scale_fill_discrete(labels = c("White fir", "Ponderosa pine"))

plot_emerge_prop
filename <- paste("../../results/figures/Simulations/prop_yearly_boxplot", name, ".png", sep = "")
...
```{r}
filename <- paste("../../results/figures/Simulations/boxplot", name, ".png", sep = "")
ggsave(file = filename, width = 4, height =4, dpi = 400)
```
