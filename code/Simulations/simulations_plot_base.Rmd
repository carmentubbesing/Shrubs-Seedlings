...
```{r}
name <- "Simulation1_2020-10-29_1000_empirical_welch_empiricalcoef_empiricalgrowth_empiricalindex"
...
dfsimallreps %>% 
  filter(!is.finite(pred_exp) | is.na(pred_exp)) %>% 
  filter(Years == 30) %>% 
  dplyr::select(rep, ID_withinrep, Sdlg, Species, Cov1.3, Ht1.3, shrubarea3, Years, climate_year)
```

```{r}
dfsimallreps %>% 
  filter(Sdlg == 205) %>% 
  filter(rep == 1 & ID_withinrep == 61) %>% 
  arrange(Years) %>% 
  View()
```
...
## Make sure # reps and minimum year matches what I expect
```{r}
...
### Look at the one that hasn't emerged
```{r}
dfsimallreps %>% 
  filter(rep == 14 & Sdlg == 71) %>% 
  dplyr::select(Years, Sdlg, Species, Ht_cm1, Ht1.3, emerged, dead) %>% 
  arrange(Years)
```
...
```{r}
df9 <- dfsimallreps %>% filter(Years == min(Years))
```
...
init_counts_by_rep <-  dfsimallreps %>% 
  ...
  count() 
ggplot(init_counts_by_rep %>% filter(Species == "PIPO"))+
  geom_histogram(aes(n))
```

There are just too few trees in the beginning! It should be 100. 
```{r}
dfsimallreps %>% 
  filter(Years == min(Years)) %>% 
  group_by(rep) %>% 
...
## Save prop summary
```{r}
save(summary_emerge_prop, file = "../../results/data/summary_emerge_prop_base_Oct29.Rdata")
```
...
ggplot(summary_emerge_prop)+
  ...
  xlim(8,max_year)+
  ylim(c(-0.05,1))
filename <- paste("../../results/figures/Simulations/base_prop_line_Oct29", name, ".png", sep = "")
ggsave(file = filename, width = 5, height =4, dpi = 400)
```
...
```{r}
save(summary_emerge_density, file = "../../results/data/summary_emerge_density_base_Oct29.Rdata")
```
...
filename <- paste("../../results/figures/Simulations/base_density_line_Oct29", name, ".png", sep = "")
ggsave(file = filename, width = 5, height =4, dpi = 400)
```
...
# Save welch ratios as a table
```{r}
write.csv(welch_ratios, file = "~/../Shrubs-Seedlings/results/coefficients/welch_ratios.csv", row.names = F)
```
