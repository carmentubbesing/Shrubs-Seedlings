...
```{r}
name <- "1_2020-04-13_1000_empirical_welch_empiricalcoef_empiricalgrowth_empiricalindex"
df_filename <- paste("~/../Ch3_Simulation_Results/Simulation", name, ".Rdata", sep = "")
load(df_filename)
strt <- Sys.time()
```
...
load("../../results/data/n_plots_welch_subset.Rdata")
...
```{r}
welch_area <-  900*plots
welch_ratios %>% 
  group_by(Species) %>% 
  summarize(trees = sum(trees)) %>% 
  mutate(density_m2 = trees/welch_area)
```

## Calculate initial area equivalent for my results

```{r}
welch_species_n <- welch_ratios %>% 
  group_by(Species) %>% 
...
calc_area <- dfsimallreps %>% 
  ...
  count() %>% 
  group_by(Species) %>% 
  summarize(mean_trees = mean(n)) 
calc_area <- full_join(calc_area, welch_species_n) %>% 
  mutate(ratio = mean_trees/trees_welch) %>% 
  mutate(area_sim = welch_area*ratio) %>% 
  summarize(area_sim = mean(area_sim))
area_sim <- calc_area$area_sim
area_sim_ha <- area_sim/10000
```
...
summary_emerge_density <- dfsimallreps %>% 
  ...
  mutate(density_ha = density*10000) %>% 
  group_by(Years, Species) %>% 
