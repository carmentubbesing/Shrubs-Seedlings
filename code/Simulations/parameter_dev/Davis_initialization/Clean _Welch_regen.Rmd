...
welch_shrspp <- welch %>% 
  ...
  filter(Shrub_species %in% c("ARPA6", "CECO", "CHSE11", "CEIN3")) %>% 
  ggplot()+
...
Choose the dominant shrub species by choosing highest shrub cover*height 
```{r}
welch_shrspp <- welch %>% 
  group_by(Regen_Plot) %>% 
...
## Repeat for shrub competition index
```{r}
welch_all %>% 
  filter(years_since_fire %in% c(5:8)) %>% 
  mutate(Shrub_species = ifelse(Shrub_species %in% c("ARPA6", "ARVI"), "ARPA/ARVI", Shrub_species)) %>% 
  group_by(Shrub_species) %>% 
  mutate(shrub_comp = sqrt(Cover*modal_ht_cm)) %>% 
  mutate(mean_comp = mean(shrub_comp)) %>% 
  ungroup() %>% 
  mutate(Shrub_species = fct_reorder(Shrub_species, desc(mean_comp))) %>%
ggplot()+
  geom_boxplot(aes(x = Shrub_species, y = shrub_comp))+
  theme_minimal()+
  geom_point(aes(x = Shrub_species, y = mean_comp), col = "red")
```
...
# Calculate per area densities

# Replace NAs with 0s because there were no trees found in the plots that say NA
```{r}
tree %>% filter(Regen_Plot == "AMR1300269")
tree %>% filter(Regen_Plot == "AMR1300270")
tree %>% filter(Regen_Plot == "AMR1300271")
```
