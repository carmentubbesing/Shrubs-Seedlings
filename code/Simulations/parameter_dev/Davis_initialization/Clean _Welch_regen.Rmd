...
```{r}
welch_subset <- welch_all %>% 
  filter(years_since_fire %in% c(5:8)) 
```


```{r}
welch_all_summary <- welch_subset %>% 
  ungroup() %>% 
  group_by(Regen_Plot) %>% 
  filter(Cover == max(Cover)) %>% 
  mutate(shrubarea = Cover*modal_ht_cm) %>% 
...
## Count the plots included in this analysis to translate later results to per area densities

```{r}
plots <- length(unique(welch_subset$Regen_Plot))
save(plots, file = "../../../../results/data/n_plots_welch_subset.Rdata")
```
