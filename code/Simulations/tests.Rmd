...
```{r}
load("~/Simulations2020-02-27_20AMRC.Rdata")
strt <- Sys.time()
```
...
test <- dfsimallreps %>% 
  group_by(Years, rep) %>% 
  mutate(sum_emerged_dead  = sum(emerged) + sum(dead), sum_emerged = sum(emerged), sum_dead = sum(dead)) %>% 
  ungroup() %>% 
  dplyr::select(rep, Years, Year, historic_year, sum_emerged_dead, sum_emerged, sum_dead) %>% 
  distinct() %>% 
  arrange(Years)
test %>% tail()
```
...
```{r}
left_join(test, df, by = c("historic_year" = "water_year")) %>% 
