...
## Sensitivity methods
```{r}
climate_method <- "uniform_2017" #historic, random, uniform_2015, uniform_2016, uniform_2017
conifer_species_method <- "random"
shrub_method <- "empirical" #emprical, CECO, ARPA, CHSE, LIDE, OTHER
```

# Iterate 
```{r, warning=FALSE}
time <- Sys.time()
dfsimallreps <- iterate(iterations, fire, years_max, climate_method, conifer_species_method, shrub_method)
...
# Save results
```{r}
filename <-  paste("~/Simulations",  Sys.Date(), "_", iterations, fire,"_", climate_method, ".Rdata", sep = "")
save(dfsimallreps, file =filename)
```
