...
AIC(update(M.interact2, . ~ . - Fire))
AIC(update(M.interact2, . ~ . - Fire-shrubarea))
AIC(update(M.interact2, . ~ . - shrubarea))
AIC(update(M.interact2, . ~ . - Fire-Fire*Ht.cm))
anova(M.interact2, (update(M.interact2, . ~ . - -shrubarea)))
AIC(update(M.interact2, . ~ . + BasDia.cm))
```
...
```{r}
anova(M.interact2, (update(M.interact2, . ~ . - Fire*Ht.cm)))
```
...
```{r}
pred <- as.data.frame(predict(M.interact2))
...
predicted <- ggplot(pred)+
  geom_point(aes(y=predict(M.interact2),x=Ht.cm,col=pred$Shrubarea, shape=pred$Fire))+
...
sse <- (dfPinus$LastYearGrth.cm - predict(M.interact2))^2
