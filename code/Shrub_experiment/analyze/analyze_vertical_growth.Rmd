...
## Throw out outlier, for which growth in 2017 is more than 5 times greater than growth in 2016
```{r}
npre <- nrow(df)
npre
df %>% filter(gr_17_over_16>5)
df <- df %>% filter(gr_17_over_16<5)
npost <- nrow(df)
npost
```

### This removed `r npre-npost` tree, bringing the df_vert total to `r npost`
