...
chse_plot <- ggplot()+
  ...
  xlab("Years since fire")+
  ylab("Shrub height (cm)")+
  ggtitle(expression(atop("", atop("Chinquapin                         ", italic("(Chrysolepis sempervirens)")))))+
  theme(
    axis.title = element_text(size = 11), 
    #plot.margin = unit(c(4,0,0,0), "cm").
    plot.title = element_text(size = 15, margin = margin(t = -2, r = 0, b = 0, l = 0, unit = "mm")))+
...
final_plot <- ggarrange(ceco_plot, arpa_plot, chse_plot, cein_plot, OTHER_plot, nrow = 3, ncol = 2)
final_plot
ggsave("~/Shrubs-Seedlings/results/figures/Shrub_growth_time/gam_fig_final.png", final_plot, width = 6, height = 8)
```
