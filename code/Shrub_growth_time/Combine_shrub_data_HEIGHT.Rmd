...
```{r}
ggplot(df_plot, aes(x = years_since_fire, y = modal_ht_cm))+
  geom_point(aes(fill = Fire, col = Fire), alpha = .5, shape = 21)+
  scale_color_manual(values = as.vector(polychrome(n = 33)[4:34]))+
  scale_fill_manual(values = as.vector(polychrome(n = 33)[4:34]))+
  theme_minimal()+
  theme(
